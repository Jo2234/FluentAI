...
class StateV2Tests(unittest.TestCase):
    ...
    def test_default_v2_shape_has_required_sections(self):
        ...
        self.assertEqual(state["active_language"], "French")
        self.assertIsNone(state["learner"]["onboarded_at"])
        self.assertIsNone(state["learner"]["last_onboarding_at"])
        self.assertIsNone(state["privacy"]["local_memory_notice_seen_at"])
        self.assertIsNone(state["preferences"]["voice_default"])
        self.assertIsNone(state["languages"]["French"]["profile"]["self_reported_level"])
        self.assertIsNone(state["languages"]["French"]["profile"]["speaking_comfort"])
        self.assertIsNone(state["languages"]["French"]["profile"]["placement_completed_at"])
        self.assertIsNone(state["languages"]["French"]["profile"]["placement_method"])
        self.assertIsNone(state["languages"]["French"]["profile"]["first_practice_goal"])
        self.assertEqual(state["languages"]["French"]["profile"]["judged_strengths"], [])
        self.assertEqual(state["languages"]["French"]["profile"]["judged_weaknesses"], [])
